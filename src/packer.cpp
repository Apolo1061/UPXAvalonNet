...
static const char *getIdentstr(unsigned *size, int small) {
    ...
    static char identbig[] =
        "\n\0"
        "$Info: "
        "This file is protected by AvalonNet AVLN-PACK $"
        "\n\0"
        "$Id: AVLN 1.0.0 Copyright (C) 2025 AvalonNet $"
        "\n";
    static char identsmall[] =
        "\n"
        "$Id: AVLN (C) 2025 AvalonNet $"
        "\n";
...
void Packer::initLoader(unsigned arch, const void *pdata, int plen, int small, int pextra) {
    ...
    // AvalonNet Polymorphism: Add a junk section with random bytes to make the binary unique
    byte junk[16];
    for (unsigned i = 0; i < sizeof(junk); i++)
        junk[i] = (byte)upx_rand();
    linker->addSection("JUNK", junk, sizeof(junk), 0);
}
